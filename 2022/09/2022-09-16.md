...
このとき `volumeClaimTemplates.spec.resources.requests.storage` により、MySQL が利用するボリュームのサイズを指定することができます。
...
これまでの coil では、新しく生成された Pod に、処理が完了した Job の Pod と同じ IP アドレスを割り当てることがありました。
その後 Job の Pod が削除される際に、coil はその Pod が持っていた IP アドレスを peer から外してしまうため、新しく生成された方の Pod まで通信ができなくなるという問題が起きていました。

CNI プラグインを実装するためには様々な状況での Pod のライフサイクルを正確に把握しなければならなず、実装の難しさを感じます。
...
なお、この CVE Feed を作成した経緯については以下の Tweet にまとまっています。
Google Groups が RSS の提供をやめたことがきっかけだそうです。

- [https://twitter.com/PuDiJoglekar/status/1569430092287471616](https://twitter.com/PuDiJoglekar/status/1569430092287471616)
